...
class ApprovalBuyerShow extends Component
{
    ...
    public $showModalViewRI = true, $showModalConfirmApprovateRI = true;
    public $showModalConfirmRejectedRI = true;
    // public $quotes;
   
    public function render()
    {
        return view('livewire.request-investment.approval-buyer-show', [
    ...
    private function getRequestInvestments()
    {
        try {
            ...
            return RequestInvestment::whereIn('RequestQuote_id',$quoteRequestIds)
                            ->whereIn('StatusList_id',[21,22,43])
    ...
    {
        try {
            $this->showModalViewRI = $this->modalOpen;
            if ($this->selectedRI) {
    ...
    public function ApprovateRI(){
        $this->selectedRI->update([
            'StatusList_id' => 22,
            'ApproveManager' => true,
            'UserApproveManager' => Auth::user()->name,
            'DateApproveManager' => Carbon::today()->format('Y-m-d'),
    ...
    public function RejectedRI(){
        ...
        session()->flash('success', 'Solicitud de inversión recha.'); 
    }
}
