...
class ApprovalBuyerShow extends Component
{
    use WithPagination;

    public $search, $selectedStatus, $selectedOrderBy = 'RFQ', $selectedOrder = 'DESC', $cadena;
    public $selectedRQ = null,  $perPage = 10;
    ...
    public function render(){
        return view('livewire.purchase-order.approval-buyer-show',[
    ...
    {
        try {
            return RequestQuote::whereIn('StatusList_id', [18, 28, 48])
    ...
    public function BuyerApprovePO(){
        $user = Auth::user();
        if($this->selectedRQ){ $this->selectedRQ->update([
            'ApprovatePOBuyer' => 1, 
            'ApprovateBuyerName' => $user->name, 
            'Gerente_id' => $user->id, 
            'ApprovateBuyerDate' => Carbon::today(),
            'StatusList_id' => 28]); 
        }
        QuoteHistory::create([
            'QuoteRequest_id' => $this->selectedRQ->id,
            'StatusList_id' => 28, //48 es el estatus de rechazo de comprador
            'remarks' => 'Aprobada por' . $user->name, 

        ]);
        ($this->selectedRQ->StatusList_id == 28) ? session()->flash('message', 'La Requisición a sido aprobada') : session()->flash('message', 'La Requisición no a podido ser aprobada');
        $this->closeAllModals();
    }
    public function BuyerRejectRQ(){
        $user = Auth::user();
        ...
        QuoteHistory::create([
            'QuoteRequest_id' => $this->selectedRQ->id,
            'StatusList_id' => 28, //48 es el estatus de rechazo de comprador
            'remarks' => 'Rechazada por' . $user->name, 
        ]);
        
        ($this->selectedRQ->StatusList_id == 48) ? session()->flash('message', 'La Requisición a sido aprobada') : session()->flash('message', 'La Requisición no a podido ser aprobada');
        $this->closeAllModals();
        $this->RQRemark = null;
    }
}
